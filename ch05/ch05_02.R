...
# filter, select, arrange, group_by, mutate(행거르기, 열거르기, 순서대로, 데이터 중 그룹핑, 연산 후 새로운 컬럼 생성)
# %>%
library(dplyr)
library(gapminder)
...
# 아프리카 대륙에서 평균 수명이 가장 긴 나라 Top5 국가
africa_pop <- filter(gapminder, continent=='Africa' &year==2007)
arrange(africa_Exp, desc(lifeExp)) %>% head(5)
...
summarize(africa_pop, pop_avg=mean(pop))    # 2007년 아프리카 국가별 평균인구수
...
----------------------------
    ...
    > mpg %>%
    +     filter(manufacture %in% c('audi','toyota')) %>%
    +     group_by(manufacturer) %>%
    +     summarize(avg_cty=mean(cty))
에러: Problem with `filter()` input `..1`.
x 객체 'manufacture'를 찾을 수 없습니다
i Input `..1` is `manufacture %in% c("audi", "toyota")`.
